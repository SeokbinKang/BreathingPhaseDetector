...
namespace BreathingPhaseModule
{
    ...
    public struct BreathingPhaseData : IBreathingPhaseDataModifier
    {
        ...
        public int phase //1~100 : inhale, -100~-1 : exhale
         {
            set;
            get;
        }
    ...
    public class BreathingPhaseAnalyzer
    {
        ...
        public BreathingPhaseAnalyzer()
        {
            ...
            lowPeakValueList = new List<double>();      //list of mins
            highPeakValueList = new List<double>();     //list of maxes
        ...
        public List <double> getLowPeaks()
        {
            return lowPeakValueList;
        }
        public List<double> getHighPeaks()
        {
            return highPeakValueList;
        }
        public void init()
        {
            ...
            lowPeakVal = -1; //default value of -1
            highPeakVal = -1;
        }

        public int pushData(double val)     //takes calibrated data point and evaluates it
        {
            curIndex++;
            if (curIndex >= paramMaxItem) curIndex = 0;     //reset index once reach 300 data points
        ...
        {
            if(nItem< paramMinGradientWindow)     //wait for gradient window (5 points) to be reached before calculating
            {
                gradientArray[curIndex] = 0;
                return 0;
            }
            double AvgGradientValue = 0;
            for(int d = 1; d <= temporalWeight.Length; d++) //loop through array of weights
            {
                double unitGradient;
                int diffIndex = curIndex - d;  //difference in indices (adjusted by weight index)
                if (diffIndex < 0)
                    diffIndex = paramMaxItem + diffIndex; //current index is less than possible weight indices, mod back to positive
                                                          
                unitGradient = (dataArray[curIndex] - dataArray[diffIndex]) / d;  //gradient (avg) over interval d
                AvgGradientValue += unitGradient * temporalWeight[d - 1]; //weighted gradient added to actual grad val ?? why

            }
            gradientArray[curIndex] = AvgGradientValue;
            return 1;
        }
        private double detectPeak() //not used
        {
            return 1;
        }
        private double calSecondDerivative()
        {
            ...
            for (int i = 1; i <= paramMinSecondDWindow;i++) {
                ...
                if (gradientArray[diffIndex] > 0) SD++; //if positive gradient (aka first derivative)
                else SD--;
            }
            if(SD==0)   //midway between min and max assuming regularity in breathing
            {
                int peakIndex = curIndex - paramMinSecondDWindow / 2;   //gets last min/max index (5 points ago)
                if (peakIndex < 0)
                ...
                if (gradientArray[curIndex]>0)
                {
                    ...
                    if (lowPeakVal == -1) lowPeakVal = dataArray[peakIndex]; //if first low peak val, set to raw value
                    else lowPeakVal = lowPeakVal * 0.5 + dataArray[peakIndex] * 0.5; //average peaks 
                } else
                {
                    //high peak
                    highPeakValueList.Insert(0, dataArray[peakIndex]);
                    if (highPeakVal == -1) highPeakVal = dataArray[peakIndex];  //if first high peak val, set to raw value
                    else highPeakVal = highPeakVal * 0.5 + dataArray[peakIndex] * 0.5; //average peaks
        ...
        {
            int result=0; //coding : 1xx : inhale, 2xx : inhale-pause , 3xx : exhale, 4xx : exhale-pause, 5xx 
            ...
            { // normal
                if (gradientVal > 0) result = 100; //positive gradient is inhale (increasing pressure)
                else result = 300;  //exhale
            }
                ...
                } else
                {
                   ...
                   // Console.WriteLine(result);               
                    ...
                    if(result/100 == 1)
                    {
                        ...
                        phaseLevel = (((dataArray[curIndex] - lowPeakVal) * 100) / (highPeakVal - lowPeakVal)); //percent distance between peaks     
                    ...
                    result += (int)phaseLevel;   //result is increased proportionally to percent within phase

                }
            }
            Console.WriteLine("Algorithm phase?: " + result);
            phaseResultList.Insert(0, result);    //add phase levels to a list
            return result;
        }
    }
}
